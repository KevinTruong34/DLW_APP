...
def do_logout():
    ...
    _logger.info(f"LOGOUT — username={user.get('username','?')}")
            ...
            if not all([u,n,p,p2]): st.error("Điền đầy đủ.")
                ...
                try:
                    supabase.table("nhan_vien").insert({
                        "username":u,"ho_ten":n,"mat_khau":hash_password(p),
...
def show_login():
    ...
    if not pending_user:
        ...
        st.markdown(
            ...
            f"<div style='font-size:1.05rem;font-weight:600;'>Xin chào, {pending_user.get('ho_ten','')}</div>"
            f"<div style='font-size:0.85rem;color:#888;'>Chọn chi nhánh để bắt đầu</div>"
            f"</div>", unsafe_allow_html=True)
        for i, branch in enumerate(branches):
            if st.button(branch, key=f"login_cn_{i}", use_container_width=True, type="secondary"):
                _finalize_login(pending_user, branch)
        st.markdown("<br>", unsafe_allow_html=True)
        if st.button("← Quay lại", key="login_back", use_container_width=True):
            st.session_state.pop("_pending_user", None); st.rerun()


# ── Auth gate — gọi từ app.py sau set_page_config ──
def run_auth_gate():
    ...
    if "active_chi_nhanh" not in st.session_state:
        ...
        if len(branches) == 1:
            ...
            save_branch_to_url(branches[0]); st.rerun()
        elif branches:
            st.markdown("<div style='padding-top: 15vh;'></div>", unsafe_allow_html=True)
            st.markdown(
                f"<div style='text-align:center;padding:20px 0;'>"
                f"<div style='font-size:1.05rem;font-weight:600;'>Xin chào, {user.get('ho_ten','')}</div>"
                f"<div style='font-size:0.85rem;color:#888;'>Chọn chi nhánh để bắt đầu</div></div>",
                unsafe_allow_html=True)
            for i, branch in enumerate(branches):
                if st.button(branch, key=f"re_cn_{i}", use_container_width=True, type="secondary"):
                    st.session_state["active_chi_nhanh"] = branch
                    save_branch_to_url(branch); st.rerun()
            st.markdown("<br>", unsafe_allow_html=True)
            if st.button("Đăng xuất", key="re_logout", use_container_width=True):
                do_logout(); st.rerun()
            st.stop()
        else:
            st.error("Tài khoản chưa được gán chi nhánh. Liên hệ admin.")
            if st.button("Đăng xuất"): do_logout(); st.rerun()
            st.stop()
