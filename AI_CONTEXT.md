...
**Cập nhật:** 14/05/2026 | **Session tiếp theo bắt đầu từ đây**
...
- **Module Chấm công — Phase 1 schema + RPC** (14/05/2026) — UI Phase 2+ chưa wire
...
| `nha_cung_cap` | NCC |
| `action_logs` | Log thao tác. **★ Patterns mới: `ADMIN_HD_CREATE`, `ADMIN_HD_EDIT`, `ADMIN_PDT_CREATE`, `ADMIN_PDT_EDIT`, `ADMIN_SC_CREATE`, `ADMIN_SC_EDIT`, `ADMIN_APSC_CANCEL`, `ATT_IN`, `ATT_OUT`, `ATT_SESSION_EDIT`** |
...
### ★ MODULE CHẤM CÔNG — Phase 1 (14/05/2026)

**Mục đích:** Schema + 5 RPC primitives nền tảng cho module chấm công xuyên 2 app (DLW + POS). Phase 1 = SQL-only, chưa wire UI.

**Files mới (commit `claude/review-guidelines-plan-hG1m4`):**
- `sql/cham_cong_p1_schema.sql` — 9 bảng + extension `btree_gist` + EXCLUDE constraint + seed 8 shift_templates + 3 branch_networks
- `sql/cham_cong_p1_rpcs.sql` — 5 RPC: `validate_check_in_pos`, `record_attendance_event`, `build_session_for_schedule`, `build_sessions_for_date`, `update_session_admin`
- `sql/cham_cong_p1_admin_edit_history_whitelist.sql` — mở rộng CHECK whitelist `admin_edit_history.table_name` thêm `'attendance_sessions'`

**Schema mới:**
- `shift_templates` (lookup, seed 8 ca: LQD×2 + GO×2 + Coop×2 + KTV×2 LQD+GO)
- `attendance_branch_networks` (seed 3 CN, `ip_prefixes TEXT[]` rỗng)
- `attendance_employee_rates` (`salary_type` enum hourly/monthly_fixed)
- `attendance_work_schedules` + EXCLUDE GIST `no_overlap_per_nv` (anti-overlap, WHERE status='scheduled')
- `attendance_events` (raw IN/OUT log từ POS)
- `attendance_sessions` (derived per schedule, status: open/completed/auto_closed/absent/edited/leave_*)
- `attendance_payroll_periods` + `attendance_payroll_items` + `attendance_adjustments`

**RPC mới (5):** theo PLAN sections 5.1-5.5.

**Pre-existing DB cleanup:** 7 bảng `attendance_*` partial (3 events / 1 session / 5 schedules test) đã DROP CASCADE trước migration để start clean (theo quyết định user).

**Wire vào app.py:** Chưa wire — Phase 2 build UI Cấu hình + Lịch.

**Smoke test passed (deploy qua Supabase MCP apply_migration):**
- shift_templates seed 8 ca ✓ ; branch_networks 3 CN ✓
- EXCLUDE constraint: chèn ca chồng → ERROR 23P01 ✓
- `validate_check_in_pos`: NV không tồn tại / ngoài khung giờ / IP whitelist rỗng / IP mismatch / IP match happy → đúng cả 5
- `record_attendance_event`: IN → event_id 1 ; sai action → "Hành động không khớp (expected OUT)" ; OUT → event_id 2 ; sau done → "Ca đã hoàn thành"
- `build_session_for_schedule`: completed, is_late=true, late_minutes đúng
- `build_sessions_for_date`: bulk `{completed: 1}`
- `update_session_admin`: snapshot before/after vào `admin_edit_history`, `fields_changed` array, status='edited', action_logs `ATT_SESSION_EDIT` level='warn`

**Decision deviation từ plan (D30):**
- `nhan_vien_id` FK dùng INTEGER (match prod `nhan_vien.id int4`) thay vì BIGINT trong plan section 4. PK riêng của attendance_* giữ BIGSERIAL.
- `_admin_validate_request` không gọi trong `update_session_admin` (caller-side `require_admin()` đã đủ); chỉ check NV exist + reason không rỗng.
- Khi schedule chưa có event và chưa qua scheduled_end_at → `build_session_for_schedule` return `{status: 'pending'}` không tạo session row (tiết kiệm row + tránh false absent).
