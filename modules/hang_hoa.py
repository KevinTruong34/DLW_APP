...
from utils.db import supabase, log_action, load_hoa_don, load_the_kho, load_hang_hoa, \
    load_phieu_chuyen_kho, load_phieu_kiem_ke, get_gia_ban_map, load_stock_deltas, \import streamlit as st
import pandas as pd
from datetime import datetime, timedelta
import numpy as np
...
def _render_them_moi():
    ...
    with c1:
        ...
        # Loại SP: Hàng hóa hoặc Dịch vụ — ảnh hưởng báo cáo XNT
        new_loai_sp = st.radio(
            "Loại SP: *",
            ["Hàng hóa", "Dịch vụ"],
            index=0,  # mặc định Hàng hóa
            horizontal=True,
            key=f"hh_new_loai_sp_{cnt}"
        )
    with c2:
